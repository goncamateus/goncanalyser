...
## The same knobs in a window: `steam-gui`

```bash
uv run steam-gui              # empty window, open a clip from the panel
uv run steam-gui voo_1.mp4    # or start on one
```

A single Qt window: video on the right with a transport bar under it, one scrollable
panel of sections on the left — *Video Source*, *Basic Adjustments* (brightness,
contrast, saturation, gamma), *Color Spaces* (BGR / gray / HSV / LAB), *Contour
Finding*, *Motion / Background Subtraction* (MOG2 or KNN, mask or background-removed)
and **Steam-Tune Controls**.

Contours come from a brightness threshold (0 hands the choice to Otsu) plus a minimum
area, drawn as outlines or as a filled mask. They are not decoration: **while the
contour section is on, the plume pipeline only keeps the pixels those contours
enclose**, and a source venting outside them drops out of the count. The percentile
thresholds are still taken over the whole frame — a region that is mostly plume would
push its own `p_hi` through the roof and then find nothing, the same reason
`plume_mask`'s `roi` argument works that way.

Same transport as `steam-tune`, so muscle memory carries over:

| key | |
|---|---|
| `space` | play / pause |
| `.` `,` | step one frame forward / back — one *sampled* frame while the plume pipeline is on |
| seek bar | jump anywhere in the clip |

The Steam-Tune section is the same pipeline as `steam-tune`, not a copy of it: the
sliders are generated by looping over the `KNOBS` table in `tune.py`, so a knob added
there appears in both front ends, and the frame cache is the same `Clip`. Decoding and
processing run on a `QThread`, so dragging a slider never freezes the window.

**Load / Save** at the bottom of the panel covers *every* knob in the window — plume
parameters, image adjustments and contour settings — in one `tune.json`. The image and
contour half goes in under a `"view"` key, which `build_config` drops on the way in
because it keeps only `Config`'s own fields, so the same file still works as
`steam-detect --config tune.json` and as `steam-tune --config tune.json`. A `tune.json`
written by the older tuner has no `"view"` block and loads with the defaults.

One thing worth knowing: the adjustment sliders never touch the plume pipeline. Its
thresholds are percentiles of the temperature index, and a gamma change would move the
percentiles the calibration was tuned against, so they only affect what you see.
